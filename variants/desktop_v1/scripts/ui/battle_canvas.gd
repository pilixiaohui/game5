...
func _ready() -> void:
	custom_minimum_size = Vector2(820, 390)
	set_process(true)
...
func _draw() -> void:
	...
	_draw_swarm(int(battle.get("biter", 0)), Vector2(contact - 170, floor_y - 20), ThemeFactory.GREEN, 1.0)
	_draw_swarm(int(battle.get("spore", 0)), Vector2(contact - 250, floor_y - 70), ThemeFactory.CYAN, 0.8)
	_draw_roots(int(battle.get("roots", 0)), Vector2(contact - 115, floor_y + 4))
	_draw_swarm(enemy_total, Vector2(contact + 145, floor_y - 20), ThemeFactory.RED, -1.0)
...
func _draw_swarm(count: int, origin: Vector2, color: Color, direction: float) -> void:
...
func _draw_roots(count: int, origin: Vector2) -> void:
	for index in range(min(8, count)):
		...
		draw_circle(point, 10.0, Color("35684f"))
