...
var new_game_confirmation: PanelContainer
...
func _show_title() -> void:
	...
	var new_game_button := UI.button("新游戏", _new_game, "PrimaryButton")
	...
	_build_new_game_confirmation(actions)
...
func _build_new_game_confirmation(parent: VBoxContainer) -> void:
	new_game_confirmation = PanelContainer.new()
	new_game_confirmation.name = "NewGameConfirmation"
	new_game_confirmation.visible = false
	parent.add_child(new_game_confirmation)
	var box := VBoxContainer.new()
	box.add_theme_constant_override("separation", 8)
	new_game_confirmation.add_child(box)
